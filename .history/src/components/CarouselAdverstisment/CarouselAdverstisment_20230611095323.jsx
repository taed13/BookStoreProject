...
const CarouselAdverstisment = () => {
  ...
  const handleSearch = (e) => {
    ...
    if (!author && !title && !keyword) {
      message.error(
        "Vui lòng điền ít nhất một trường tìm kiếm"
      );
      return;
    }
  ...
  const handleClick = async () => {
    console.log("data", author, title, price);
    const res = await searchAPI.searchByMultipleParams(
      price.min,
      price.max,
      author,
      title
    ...
    if (res.status === 200) {
      console.log("res ---> ", res);
      context.setSearchDataResult(res.data);
      navigate("/search");
    } else {
                ...
                <div className='carousel-item active'>
                  ...
                  <div className='col-md-12 col-lg-10 alert-light position-absolute bottom-0 left-0 rounded-right pt-4 pr-4'>
                    <div className='position-relative h-100'>
                      ...
                      >
                        The world's most valuable children's
                        books
                      </Link>
                      <p className='py-2'>
                        Children is books can be valuable if
                        they are the right edition in the
                        right condition. Discover 10 books
